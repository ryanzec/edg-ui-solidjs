...
const Icon = (passedProps: IconProps) => {
  const [props, restOfProps] = splitProps(
    ...
    ['class', 'icon', 'size', 'color', 'isClickable', 'variant', 'disabled'],
  );

  return (
    <button
      data-id="icon"
      class={tailwindUtils.merge(`inline-flex`, styles.icon, props.class, {
        ...
        [styles.clickable]: props.disabled === false && (!!restOfProps.onClick || props.isClickable),
    ...
    >
      <i
        {...restOfProps}
        class={tailwindUtils.merge(`ph-${props.icon} font-inherit`, {
