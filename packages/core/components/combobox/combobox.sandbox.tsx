...
const formDataSchema = zodUtils.schemaForType<FormData>()(
  zod.object({
    combobox: zod
      .number()
      .array()
      .min(1, validationUtils.getMessage(ValidationMessageType.MINIMUM_COUNT, ['1', 'option'])),
  }),
);
...
export const SingleInForm = () => {
  ...
  return (
    <form use:formDirective>
      <BasicExample onSelected={onSelected} errors={form.errors().combobox?.errors} />
...
export const MultiInForm = () => {
  ...
  return (
    <form use:formDirective>
      <MultiSelectExample onSelected={onSelected} errors={form.errors().combobox?.errors} />
      <button type="submit">Submit</button>
    </form>
  );
};
...
export const SingleInFormAutoShowOptions = () => {
  ...
  return (
    <form use:formDirective>
      <BasicExample onSelected={onSelected} autoShowOptions errors={form.errors().combobox?.errors} />
      <button type="submit">Submit</button>
    </form>
  );
};
...
export const MultiInFormAutoShowOptions = () => {
  ...
  return (
    <form use:formDirective>
      <MultiSelectExample onSelected={onSelected} autoShowOptions errors={form.errors().combobox?.errors} />
      <button type="submit">Submit</button>
    </form>
  );
};
...
const NameTypingTest = () => {
  const form = formStoreUtils.createStore<{ combobox: number[] }>({
    onSubmit: async (values) => {
      console.log(values);
    },
  });
